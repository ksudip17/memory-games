...
export default function SignalBoard({
  ...
  if (gameStatus === "idle") {
    return (
      <div className="bg-gradient-to-br from-slate-50 to-slate-100 rounded-2xl p-4 sm:p-6 md:p-8 lg:p-12 mb-4 sm:mb-6 text-center border border-slate-200">
        <p className="text-slate-600 text-xs sm:text-sm md:text-base lg:text-lg">Click Start to begin the challenge</p>
      </div>
    )
  }
  ...
  return (
    <div className={`${getBgColor()} rounded-2xl p-4 sm:p-6 md:p-8 mb-4 sm:mb-6 border border-slate-200 transition-colors`}>
      <div className="mb-4 sm:mb-6 text-center">
        <div className="inline-block px-3 py-2 sm:px-4 sm:py-2 md:px-6 md:py-3 rounded-xl font-bold text-xs sm:text-sm md:text-lg bg-gradient-to-r from-purple-500 to-pink-500 text-white animate-pulse break-words max-w-[95%] sm:max-w-full">
          {currentQuestion}
        </div>
      </div>

      <div className="flex flex-wrap justify-center items-center gap-3 sm:gap-4 md:gap-6 mb-4 sm:mb-6 min-h-[120px] sm:min-h-[150px] md:min-h-[180px]">
        {currentOptions.map((option) => (
          <button
            ...
            disabled={hasClicked}
            className={`
              text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold border-2 md:border-4 rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-6 
